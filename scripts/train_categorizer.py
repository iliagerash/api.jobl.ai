...
Training augmentation:
    Every row is duplicated with original_category blanked out, so the model
    learns to predict from title + description alone when original_category is
    absent at inference time (doubling the effective training set size).
...
def main() -> None:
    ...
    # Augment: add a copy of every row with original_category blanked out so the
    # model learns to classify from title + description alone when original_category
    # is absent at inference time.
    df_masked = df.copy()
    df_masked["original_category"] = ""
    df_train = pd.concat([df, df_masked], ignore_index=True)
    print(f"  {len(df_train)} rows after augmentation (2× with original_category masked)")
